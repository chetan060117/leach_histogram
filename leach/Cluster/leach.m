function [nodeArch, clusterNode, clusterEnergy] = leach(clusterModel, clusterFunParam)
    ...
    % Initialize clusterEnergy if it doesn't exist
    if ~isfield(clusterModel, 'clusterEnergy')
        clusterModel.clusterEnergy = cell(1, r); % Cell array to store energy levels for each round
    end
    ...
    % Store the energy levels of clusters for this round
    clusterEnergy{r} = zeros(1, countCHs);
    for i = 1:countCHs
        chNo = clusterNode.no(i);
        clusterEnergy{r}(i) = nodeArch.node(chNo).energy;
    end
    
    % Update clusterModel with cluster energy information
    clusterModel.clusterEnergy{r} = clusterEnergy{r};
end
